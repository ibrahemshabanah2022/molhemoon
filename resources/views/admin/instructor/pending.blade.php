                        ...
                        <div class="breadcrumb__content__right">
                            <nav aria-label="breadcrumb">
                                <ul class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">{{ __('Pending Instructor') }}
                                    </li>
                                </ul>
                            </nav>
                        </div>
                                ...
                                <tbody>
                                    @foreach ($instructors as $instructor)
                                        <tr class="removable-item">
                                            <td>
                                                <a href="{{ route('instructor.view', [$instructor->uuid]) }}"> <img
                                                        src="{{ getImageFile($instructor->user ? $instructor->user->image_path : '') }}"
                                                        width="80"> </a>
                                            </td>
                                            <td>
                                                {{ $instructor->name }}
                                            ...
                                            <td><span><a href="{{ getVideoFile($instructor->cv_file) }}"
                                                        target="_blank">{{ $instructor->cv_filename }}</a></span></td>

                                            <td>
                                                <div class="action__buttons">
                                                    <a href="{{ route('instructor.status-change', [$instructor->uuid, 1]) }}"
                                                        class="btn-action approve-btn mr-30 p-1" title="Make as approve">
                                                        {{ __('Approve') }}
                                                    </a>
                                                    <a href="{{ route('instructor.view', [$instructor->uuid]) }}"
                                                    ...
                                                    <a href="javascript:void(0);"
                                                        data-url="{{ route('instructor.delete', [$instructor->uuid]) }}"
